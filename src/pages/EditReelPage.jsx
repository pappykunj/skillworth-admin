...
const EditReelPage = () => {
  ...
  const fetchReel = useCallback(async () => {
    try {
      const response = await apiClient.get(`/admin/get/reel/${id}`);
      const reel = response.data.reel;
      setTitle(reel.title);
      setDescription(reel.description);
      setSelectedUser(reel.user._id);
      setSelectedSkills(reel.skillId.map(skill => skill._id));
      setSelectedSubSkills(reel.subSkillsId.map(subSkill => subSkill._id));
    } catch (error) {
  ...
  useEffect(() => {
    ...
    fetchUsers();
    fetchReel();
  }, [fetchSkillsAndSubSkills, fetchUsers, fetchReel]);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      await apiClient.put(`/admin/update/reel/${id}`, formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });
      setSnackbar({ open: true, message: 'Reel updated successfully!', severity: 'success' });
      setTimeout(() => navigate('/dashboard/reels'), 2000);
    } catch (error) {
      console.error("Failed to update reel:", error);
      setSnackbar({ open: true, message: error.response?.data?.message || 'Failed to update reel.', severity: 'error' });
    }
  };
